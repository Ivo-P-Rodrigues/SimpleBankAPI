...
namespace SimpleBankAPI.WebAPI.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<ActionResult<CreateUserResponse>> Create(CreateUserRequest createUserRequest)
        {
            ...
            _logger.LogInformation("Testing log: user created");
            return Created("", userResponse); //no URI... no rest...
        }
        ...
        {
            if (!_userBusiness.VerifyUserCredentials(loginUserRequest.Password, loginUserRequest.Username, out int userId))
        ...
        public async Task<ActionResult> Logout(LogoutUserRequest logoutUserRequest)
        {
            ...
            var userId = int.Parse(User.Claims.FirstOrDefault(c => c.Type == "userId")?.Value!);
            if(!_sessionBusiness.VerifyLogoutRequestObj(userId, logoutUserRequest, out Session session))
