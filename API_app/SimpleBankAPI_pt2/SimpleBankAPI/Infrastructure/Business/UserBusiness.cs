...
namespace SimpleBankAPI.Business
{
    public class UserBusiness : IUserBusiness
    {
        ...
        public bool VerifyUserCredentials(string password, string username, out int userId)
        {
            var user = _unitOfWork.Users.FirstOrDefault(u => u.Username == username); //usernames are unique and not null
            if (user == null)
            { userId = 0; return false; } //0 to avoid int?
